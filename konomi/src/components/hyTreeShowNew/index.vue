<template>
    <div class="treeBox">
        <div class="head" @click="onClickNode({})">
            <div class="treeShowTitle">{{headTitle}}</div>
            ...
            <div class="firstLevel" v-for="(item,index) in levelData" :key="index">
                <div class="levelContain">
                    <div class="top" @mouseenter="enterArea" @mouseleave="leaveArea">
                        <!-- <scroll-lock class="rightArea1" v-if="item[obj.label]=='缓冲区（BUF）'&&areaShow">
                            <div class="nodeContain">
                                <div class="right_top" v-for="(ele, el) in item.children" :key="el" @click="onClickNode(ele)">
                                    <div class="items">
                                        {{ele[obj.label]}}<span class="countNum">{{ele.count}}</span>
                                    </div>
                                </div>
                            </div>
                        </scroll-lock> -->
                        <scroll-lock class="rightArea" v-if="areaShow" :style="{height: viewHeight+'px'}" :class="positionTop?'rightAreaTop':'rightAreaDown'">
                            <div class="itemsArea" v-for="(ele, el) in item.children" :key="el">
                                <div class="right_top" @click="onClickNode(ele)">{{ele[obj.label]}}</div>
                                <div class="right_bottom">
                                    <div class="items">
                                        <div
                                            class="item"
                                            @click="onClickNode(i,item[obj.label])"
                                            v-for="(i,j) in ele.children"
                                            :key="j"
                                        >{{i[obj.label]}}<span class="countNum2" @click.stop="onClickNode(i,item[obj.label])">{{i.count}}</span></div>
                                    </div>
                                </div>
                            </div>
                        </scroll-lock>
                        <div class="levelName" @click="onClickNode(item);showIdClick(item)">{{item[obj.label]}}</div>
                        <div class="total">{{item.count}}</div>
                    </div>
                    <div class="bottom" v-if="type == 'service'">
                        <div
                            class="bottom_item"
                        ...
                        >
                            ...
                            <div class="rightArea2" v-if="ele.children&&areaShow" :style="{maxHeight: viewHeight+'px'}" :class="positionTop?'rightAreaTop':'rightAreaDown'">
                                <div class="nodeContain">
                                    <div class="right_top" v-for="(i,j) in ele.children" :key="j" @click.stop="onClickNode(i,item[obj.label])">
                                        <div class="items">
                                            <span>{{i[obj.label]}}</span>
                    ...
                    <div class="bottom" v-if="type == 'system' && item.id == showId">
                        <div
                            class="bottom_item"
                            v-for="(ele, el) in item.children"
                            :key="el"
    ...
    methods: {
		...
		onClickNode(node) {
            ...
            if(this.type == 'service') {
                ...
                this.$emit('onClickNode',nodeId,nodeType,nodeTitle)
            }else if(this.type == 'system'){
                if (node.id) {
                    nodeId = node.id;
				    nodeType = node.sys_id;
                    nodeTitle = node.parentNames;
                ...
                this.$emit('onClickNode',nodeId,nodeType,nodeTitle,parentId)
            }
        },
        enterArea(e) {
            let top = e.pageY - 150
