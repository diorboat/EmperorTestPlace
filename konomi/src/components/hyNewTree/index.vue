      ...
      <el-tree :accordion='accordion' :data="treeData" :props="defaultTreeProps" @node-click="treeClick" :show-checkbox='showTreeCheckBox' @check-change="treeSelect" :draggable='draggable' :filter-node-method="filterNode" :node-key="treeNodeKey" :default-expand-all='ifExpandAll' :expand-on-click-node='ifClickNodeExpand' :check-strictly='checkStrictly' @node-expand='nodeExpand' ref='hyTree'>
        <span class='treeSlot' slot-scope="{node, data}" @mousemove="mousemove(data[treeNodeKey])" @mouseleave="mouseleave(data[treeNodeKey])">
          ...
          <span v-if='ifCanDoSomethings.ifPromise && data[treeNodeKey]==mouseHoverNode'>
            <img v-if='ifCanDoSomethings.operation.append && (!Number(data.is_leaf) || (Number(data.is_leaf) && Number(data.is_delete)))' alt="" src="../../assets/tableIcon/add_2_hover.png" style="width: 14px; height:14px; margin: 0 2px;" @click="append(node, data)">
            <img v-if='ifCanDoSomethings.operation.edit&&!node.closeEdit' alt="" src="../../assets/tableIcon/editer_2_hover.png" style="width: 14px; height:14px; margin: 0 2px;" @click="edit(node, data)">
            <img v-if='ifCanDoSomethings.operation.remove&&Number(node.is_delete)' alt="" src="../../assets/tableIcon/delete_2_hover.png" style="width: 14px; height:14px; margin: 0 2px;" @click="remove(node, data)">
