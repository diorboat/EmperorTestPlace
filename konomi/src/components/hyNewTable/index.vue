<template>
	<div class="hyTable_box">
		<div class="hyTable">
			<el-table border :ref='tableRef' :data="formatData" stripe :highlight-current-row="highlightCurrentRow" :row-key="getRowKeys" @cell-click='cellClick' @cell-dblclick='cellDblClick' @sort-change='sortChange' @row-click='rowClick' :height='height' :max-height='maxHeight' @selection-change="handleSelectionChange" @expand-change="handleExpandChange" :row-style="showRow" :row-class-name="rowClassMetohd" v-bind="$attrs" :header-cell-style='headerCellStyle' :cell-style='cellStyle' :size='size' :style="tableStyle">
				<!-- 是否多选 -->
				...
				<!-- table数据 -->
				<el-table-column v-for='(item) in column' :prop="item.key" :key='item.key' :sortable='item.sortable' :reserve-selection="ifHaveCheckBox" :label='item.label' :align='item.align' :width="item.width" :show-overflow-tooltip=true >
				...
				<slot></slot>
				<!-- 修改删除操作 -->
				<el-table-column label='操作' align='left' :width='handleWidth' v-if='isHaveOds||isHaveDw||isHaveOpen||isHaveEdit||isHaveDelete||isHaveSuspend||isHaveRecovery||isHaveAdd||isHaveChangePass||isHaveRoad || isHaveDownload || isHaveUpload'>
					<template slot-scope="scope">
						...
						<el-tooltip content="下载" placement="top" v-if='isHaveDownload'>
							<span @click.stop='download(scope)' class="click">
								<img class="imgCom" src="../../assets/rousourceInventory/download.png">
								<img class="imgSel" src="../../assets/rousourceInventory/download_hover.png">
							</span>
						</el-tooltip>
						<el-tooltip content="上传" placement="top" v-if='isHaveUpload'>
							<span @click.stop='upload(scope)' class="click">
								<img class="imgCom" src="../../assets/rousourceInventory/upload.png">
								<img class="imgSel" src="../../assets/rousourceInventory/upload_hover.png">
							</span>
						</el-tooltip>
						...
						<el-tooltip content="执行" placement="top">
							<span class="click" v-if='isHavePerform' @click.stop='perform(scope)'>
								<img class="imgCom" src="../../assets/dataResourceCollect/perform.png">
								<img class="imgSel" src="../../assets/dataResourceCollect/perform_hover.png">
							</span>
						</el-tooltip>
						<el-tooltip content="日志" placement="top">
							<span class="click" v-if='isHavelog' @click.stop='jumpLog(scope)'>
								<img class="imgCom" src="../../assets/dataResourceCollect/log.png">
								<img class="imgSel" src="../../assets/dataResourceCollect/log_hover.png">
							</span>
						</el-tooltip>
					</template>
				</el-table-column>
...
export default {
	...
	methods: {
		...
		openHandle(scope) {
			this.$emit('openHandle', scope.row,'open')
		},
		openDw(scope) {
		...
		editHandle(scope) {
			this.$emit('editHandle', scope.row, 'edit')
		},
		download(scope) {
			this.$emit('download', scope.row)
		},
		upload(scope) {
			this.$emit('upload', scope.row)
		},
		deleteHandle(scope) {
			this.$confirm('是否删除?',{
		...
		perform(scope){
			this.$emit('perform', scope)
		},
		jumpLog(scope){
			this.$emit('jumpLog', scope)
		},
		...
		handleSelectionChange(val) {
			this.$emit('handleSelectionChange', val,this.$refs[this.tableRef])
		},
