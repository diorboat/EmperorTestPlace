<template>
	<div class="newForm_box">
		<el-form ref="form" class="newForm" :rules="rules" :model="formData" :size="size" :label-width="labelWidth" :disabled="disabled" @submit.native.prevent :validate-on-rule-change="validateOnRuleChange">
			<!-- :inline="ifInLine" -->
			...
			<el-form-item v-for="item in formConfig" :key="item.key + item.label" :label="item.label" :prop="item.key"  :style="{'width':item.width}" :label-width="item.labelWidth || labelWidth">
				...
				<div v-if="!item.notShow" class="item_box" :style="{'width': item.itemWidth ||'100%'}">
					<el-tooltip :disabled="!((item.tip || '').length>0)" :content="item.tip" placement="top" effect="dark">
						...
						<!-- textarea -->
						<el-input v-if="item.type === 'textarea'" type="textarea" v-model="formData[item.key]" :placeholder="item.placeholder" :disabled="item.disabled" :rows="item.rows" style="width:100%" :readonly="item.readonly" @keyup.enter.native='textareaEnter(item.key,formData[item.key])'></el-input>
						<!-- select -->
						...
						<!-- <el-tooltip v-if="item.type == 'cascader'" :content="formData[item.key]">
							<el-cascader style="width:100%" change-on-select :show-all-levels="showAllLevels" :clearable="item.clearable" :placeholder="item.placeholder" :options="item.options" :props="item.props" :disabled="item.disabled || item.readonly" v-model="formData[item.key]" @click.native="formSelectClick(item.key)" @change="formcascchange"></el-cascader>
						</el-tooltip> -->
						<!-- transfer -->
						...
						<!-- switch -->
						<el-switch v-if="item.type == 'switch'" v-model="formData[item.key]" :inactive-text="item.title" :active-value="item.activeValue" :inactive-value="item.inactiveValue"></el-switch>
						...
						<!-- timepicker -->
						<el-time-picker v-if="item.type == 'timepicker'" v-model="formData[item.key]" :type="item.dateType" :placeholder="item.placeholder" :format="item.format" :value-format="item.valFormat" :readonly="item.readonly" :disabled="item.disabled" style="width:100%"></el-time-picker>
						<!-- daterange -->
						...
						<el-checkbox-group v-if="item.type==='checkboxgroup'" v-model="formData[item.key]">
							<el-checkbox v-for="v in item.options" :key="v.label" :label="v.label" :disabled="item.disabled || item.readonly">{{v.text}}</el-checkbox>
						</el-checkbox-group>
						<!-- button -->
		...
		<div class="searchButton">
			<el-button v-for="item in searchButtonsConfig" :size="size" :key="item.key" :type="item.type" @click="searchButtonClick(item.key,formName)">{{item.text}}</el-button>
		</div>
		<!-- <div class="searchButton">
 ...
 * formConfig
 * 通用配置
 * @param {validateOnRuleChange} 表单校验规则发生变化时是否需要立即验证
...
export default {
	...
	methods: {
		...
		textareaEnter(key,val){
			this.$emit("textareaEnter",key,val);
		},
		...
		searchButtonClick(item) {
			this.$emit("searchButtonClick", item, this.formName);
		}
	},
	watch: {
		clearAll(val) {
