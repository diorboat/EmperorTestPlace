...
export default {
  ...
  mounted() {
    if (this.CheckIsColor(localStorage.getItem("colorPicker"))) {
      ...
      this.theme = this.themeColor.hy_element;
    }
  },
  watch: {
    'themeColor.hy_element'(v) {
      this.theme = v
    },
    theme(val, oldVal) {
  ...
  methods: {
    elementChange(e) {
      this.setColorSelectV('')
      window.localStorage.removeItem('colorSelectV')
      this.theme = e ? e : this.theme = this.themeColor.hy_element;
    },
    CheckIsColor(bgVal) {
      if(bgVal){
